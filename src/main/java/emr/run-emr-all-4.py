...
clusterName = "AUTO_CLUSTER_" + "computint-all play_req_daily_data + play_hourly_count" 
...
client.run_job_flow(
    ...
    Steps=[
        ...
        {
            'Name': 'play_req_daily_data',
            'ActionOnFailure': 'TERMINATE_JOB_FLOW',
            'HadoopJarStep': {
                'Jar': 's3://cn-north-1.elasticmapreduce/libs/script-runner/script-runner.jar',
                'Args': [
                    's3://fengxinzi-bucket01/emr_conf/compute-step/run.sh',
                    '-j', 'ott.play.play_req_daily_data',
                    '-s', '2016-01-07',
                    '-e', '2016-01-24'
                ]
            }
        ...
        {
            'Name': 'play_hourly_count',
            'ActionOnFailure': 'TERMINATE_JOB_FLOW',
            'HadoopJarStep': {
                'Jar': 's3://cn-north-1.elasticmapreduce/libs/script-runner/script-runner.jar',
                'Args': [
                    's3://fengxinzi-bucket01/emr_conf/compute-step/run.sh',
                    '-j', 'ott.play.play_hourly_count',
                    '-s', '2016-01-07',
                    '-e', '2016-01-24'
                ]
            }
