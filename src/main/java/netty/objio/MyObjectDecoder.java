...
public class MyObjectDecoder extends ByteToMessageDecoder{
	...
	static int HEAD_LEGNTH = 4;

	@Override
	protected void decode(ChannelHandlerContext ctx, ByteBuf in, List<Object> out) throws Exception {
		//这个HEAD_LENGTH是我们用于表示头长度的字节数。  由于MyObjectEncoder先writeInt(dataLength)，所以这里HEAD_LENGTH的值为4.
		if (in.readableBytes() < HEAD_LEGNTH) {
			return;
		}
	...
	private Object convertToObject(byte[] body) {
		return JsonUtil.fromJson(body, Person.class);
	}
